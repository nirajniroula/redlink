...
const searchPlaces = async (country: string, query: string) => {
  try {
    const response = await fetch(
      `http://api.geonames.org/searchJSON?name_startsWith=${query}&country=${country}&maxRows=10&username=niniroula.dev`,
    );
...
export { getLocation, searchPlaces };
